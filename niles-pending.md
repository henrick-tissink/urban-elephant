...
- **Docklands has no Nightsbridge listing of its own — decide what we want.** Trystan (NB, 3 Aug) listed only three BBIDs for the group: Bree 30034, Rose 39237, Flamingo 39239. He confirmed Docklands inventory sits *inside* 30034 (he quoted the unit "Urban Elephant 1317 (43) — Apartment 317 … 13th floor of The Docklands"). So the Docklands page isn't mis-linked, but a guest who clicks Book Direct on `/properties/the-docklands` lands on a booking page branded **16 On Bree**. Either ask NB to split Docklands onto its own BBID, or accept it and say so on the page. Needs a call from Niles/Karin.
- **Keep the source tags off our own links.** Now that the bare URL = "own website", any link *not* on urbanelephant.co.za must carry an nbid or it will pollute the direct figure. Instagram bio stays `?nbid=1040` (that ID is registered as Instagram). NB gave us three spare source slots we can point at Google Business Profile / newsletter: `?nbid=550` (labelled "Alternative Rate"), `?nbid=563` ("Alternative Source"), `?nbid=619` ("Alternative Currency") — the labels can't be renamed, so we just need to record which is which.
- **The Rose / Nightsbridge B&W** — confirmed code-side: all four properties hit Nightsbridge via the same URL pattern `https://book.nightsbridge.com/{id}` with no theming params on our end. Bree (30034), Rose (39237), Docklands (30034), Flamingo Express (39239). The colour-vs-B&W rendering for The Rose is a Nightsbridge-side property setting on ID 39237 — Karin needs to flip it in the NB admin panel (or raise a ticket with NB support). Off our list, same pattern as the earlier 317 / 13th-floor issue.

Resolved over the past sessions:

- **Website bookings now report as Direct (12 Aug)** — every booking link on the site carried `?nbid=1040`, which Nightsbridge has registered as **Instagram**, so 100% of our website's bookings were being reported back to us as Instagram bookings and the channel report had no Direct row at all. Trystan confirmed the "own website" source is simply the URL with no `?nbid=` on it. `nbid=1040` stripped from all ten links — the four property `bookingUrl`s and `siteSettings.bookNowUrl` in `src/data/content.ts`, plus the promo `ctaHref` in all five locale files. From the next deploy, site bookings land in Nightsbridge as "own website".
