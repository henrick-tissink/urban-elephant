...
const applicationSchema = z.object({
  ...
  numberOfUnits: z.coerce.number().min(1, "Please enter the number of units"),
...
function buildHtml(data: ApplicationPayload): string {
  const { name, email, phone, propertyLocation, numberOfUnits, propertyDescription } = data;
  const row = (label: string, value?: string | number) =>
    value !== undefined && value !== ""
      ? `<tr><td style="padding:8px 0;color:#6b6b6b;font-size:13px;width:160px;vertical-align:top">${esc(label)}</td><td style="padding:8px 0;color:#24272a;font-size:14px;vertical-align:top">${esc(String(value))}</td></tr>`
      : "";
        ...
        <table role="presentation" width="600" cellspacing="0" cellpadding="0" border="0" style="max-width:600px;background:#ffffff;border:1px solid #ececec;">
          <tr>
            <td style="background:#24272a;padding:24px 32px;">
              ...
              <p style="margin:6px 0 0;color:#ffffff;font-size:20px;font-weight:300;">New property application</p>
            </td>
          </tr>
          <tr>
          ...
          <tr>
            <td style="padding:8px 32px 20px 32px;">
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">
                ${row("Property location", propertyLocation)}
                ${row("Number of units", numberOfUnits)}
              </table>
            </td>
          </tr>
          <tr>
            <td style="padding:0 32px 24px 32px;">
              <p style="margin:0 0 8px;color:#6b6b6b;font-size:12px;letter-spacing:0.2em;text-transform:uppercase;">Property description</p>
              <div style="padding:16px;background:#f8f8f8;border-left:3px solid #ff00ff;color:#24272a;font-size:15px;line-height:1.55;white-space:pre-wrap;">${esc(propertyDescription)}</div>
            </td>
          </tr>
          <tr>
            <td style="padding:0 32px 32px 32px;">
              <a href="mailto:${esc(email)}?subject=${encodeURIComponent("Re: Your Urban Elephant property application")}" style="display:inline-block;background:#ff00ff;color:#ffffff;padding:12px 22px;text-decoration:none;font-size:13px;letter-spacing:0.12em;text-transform:uppercase;font-weight:700;margin-right:8px;">Reply by email</a>
...
export async function POST(request: NextRequest) {
  ...
  try {
    const { error } = await resend.emails.send({
      ...
      text: buildText(data),
      replyTo: data.email,
      tags: [{ name: "source", value: "property-application" }],
    });
    ...
    console.error("Property application error:", err);
    return NextResponse.json(
